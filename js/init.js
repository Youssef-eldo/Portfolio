$.fn.eldeoProgressIsInViewport = function(content) {
	"use strict";
...
(function($){
  "use strict";
	...
	var Frenifyeldeo = {
		...
		init: function(){
			Frenifyeldeo.BgImg();
			Frenifyeldeo.imgToSVG();
			Frenifyeldeo.progress();
			Frenifyeldeo.resume();
			Frenifyeldeo.loadBlogPosts();
			Frenifyeldeo.modal();
			Frenifyeldeo.progress2();
			Frenifyeldeo.contactForm();
			Frenifyeldeo.toTopJumper();
			Frenifyeldeo.rating();
			Frenifyeldeo.isotope();
			Frenifyeldeo.portfolioFilter();
			Frenifyeldeo.magnific();
			Frenifyeldeo.anchor();
			Frenifyeldeo.darklight();
		},
		
		darklight: function(){
			$('.eldeo_fn_switcher_wrap input').on('change',function(){
				var checkBox = $(this);
		...
		progress2: function(){
			$('.fn_cs_progress_bar').each(function() {
				var pWrap 	= $(this);
				pWrap.waypoint({handler: function(){Frenifyeldeo.progress2F(pWrap);},offset:'90%'});
			});
		},
		...
		modal: function(){
			var self		= this;
			var modalBox	= $('.eldeo_fn_modalbox');
			var item		= $('.modal_item');
		...
		modal_prevnext: function(prevNext,modalBox){
			...
			prevNext.find('a').off().on('click',function(){
				...
				$(".eldeo_fn_modalbox .modal_content").stop().animate({scrollTop:0}, 500, 'swing');
		...
		resume: function(){
			$('.eldeo_fn__cv .cv__content').scrollTop(0);
			$('body').addClass('resume-opened');
		},
		
		progress: function(){
			var content = $('.eldeo_fn__cv .cv__content');
			if($(window).width() <= 768){
				content = $('.eldeo_fn__cv');
			}
			content.on('resize scroll', function() {
				if ($('.eldeo_fn__cv .fn_cs_progress_bar').eldeoProgressIsInViewport(content) < 0) {
					Frenifyeldeo.progressF($('.eldeo_fn__cv .fn_cs_progress_bar'));
				}
			});
		},
	...
	// READY Functions
	$(document).ready(function(){Frenifyeldeo.init();});
